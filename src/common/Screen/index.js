...
export default class Screen extends Component<Props, State> {
  ...
  updateHeaderHeight = () => {
    if (this.headerRef && this.headerRef.current && this.headerRef.current.clientHeight) {
  ...
  render() {
    const { background, backButton, children, title, headerContent, contentWidth, headerRight, isFullWidth, padding } = this.props;
      ...
      <ScreenWrapper>
        <ScreenHeader scrolledFromTop={scrolledFromTop}>
          <div ref={this.headerRef}>
            <Fade duration={200}>
              <Container isFullWidth={isFullWidth} padding={padding} width={contentWidth ? contentWidth : 1400}>
                <Flex justify="space-between" align="center">
                  {title && <h1>{title}</h1>}
                  {headerRight && <TitleRight scrolledFromTop={scrolledFromTop}>{headerRight}</TitleRight>}
                </Flex>
                {backButton && (
                  <div>
                    <Spacing height={10} />
                ...
                {headerContent && (
                  <ScreenHeaderContent scrolledFromTop={scrolledFromTop}>
                    <Spacing height={30}/>
                    {headerContent}
                  </ScreenHeaderContent>
                )}
        ...
        <ScreenContent onScroll={this.handleScroll} headerHeight={headerHeight} scrolledFromTop={scrolledFromTop} background={background}>
          <ScreenScrollContent>
            <Fade bottom distance="4px" delay={200}>
              <Container width={contentWidth ? contentWidth : 1400} isFullWidth={isFullWidth} padding={padding}>
                {children}
              </Container>
            </Fade>
          </ScreenScrollContent>
